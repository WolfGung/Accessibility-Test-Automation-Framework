...
`vendor/README.md` names the version and the checksum; the file on disk must
match both, and the licence next to it must be the one its banner names.
"""
...
def test_axe_min_js_matches_the_checksum_in_the_readme() -> None:
    ...
    assert hashlib.sha256((VENDOR / "axe.min.js").read_bytes()).hexdigest() == pinned


def test_the_banner_and_the_readme_name_the_pinned_version() -> None:
    banner = banner_of((VENDOR / "axe.min.js").read_text(encoding="utf-8"))
