...
@pytest.mark.parametrize("violation", AXE_FINDS, ids=[violation.id for violation in AXE_FINDS])
def test_axe_finds_each_violation_the_registry_says_it_finds(
    pages: Pages, broken_shop: str, violation: Violation
) -> None:
    # Its page as it opens (a dialog's backdrop over the page hides the price's background from axe); for a
    # violation on every page, every state.
